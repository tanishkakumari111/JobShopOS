...
async function main() {
  ...
  assert(mode === "database", "JOBSHOP_DATA_SOURCE=database is required for the report command route smoke test.");
  assert(Boolean(process.env.DATABASE_URL?.trim()), "DATABASE_URL is required for the report command route smoke test.");

  const { route: idempotencyKey } = getReportSmokeKeys();
  ...
  assert(
    ...
    "Expected report route replay behavior to be observable through replayed=true or a stable commandId."
  );

  console.log("Route smoke test passed:");
  console.log("- POST /api/commands/reports/customer-status/j-2035/generate succeeded");
